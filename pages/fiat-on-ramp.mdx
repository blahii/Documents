...
- **`ticker`**: Тикер. Необязательный параметр если передан currency.
...
- **`email`**: Почта пользователя для автоматической вставки в виджет. Необязательный параметр.
- **`lang`**: Язык формы.
