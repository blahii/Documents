# API Reference

## Аутентификация и формат запросов

Все запросы к API требуют аутентификации через подпись запроса для обеспечения безопасной передачи данных.

### Структура запроса

Все запросы отправляются методом **POST**. Тело запроса содержит два обязательных поля:

- **`data`**: JSON-объект, закодированный в base64
- **`signature`**: Подпись запроса (SHA256(`SECRET_KEY` + `data`)), закодированная в base64

---

### Создание заказа
**URL**: `https://crypto-cash.world/merchant/api/v1/orders/create/`

> **Важно**: При создании заказа сумма передается в фиатной валюте

#### Параметры запроса:
- **`publicKey`**: Публичный ключ мерчанта
- **`amount`**: Сумма заказа
- **`currency`**: Код фиатной валюты
- **`number`**: Уникальный идентификатор заказа
- **`redirectUrl`**: Ссылка для перенаправления после оплаты. Не обязательный параметр.

В ответ сервер вернёт url (`paymentUrl`) для перенаправления пользователя на страницу оплаты.

#### Пример ответа:
```json
{
  "code": 200,
  "data": {
    "item": {
      "paymentUrl": "https://buy-crypto.cash/merchant/order/ee6b7352-8e8a-43ad-a09e-b2f48c336e90/"
    }
  }
}
```

---

### Получение списка заказов
**URL**: `https://crypto-cash.world/merchant/api/v1/orders/list/`

#### Параметры запроса:
- **`publicKey`**: Публичный ключ мерчанта
- **`page`** (опционально): Номер страницы (по умолчанию: 1)
- **`pageSize`** (опционально): Элементов на странице (по умолчанию: 100)

#### Пример ответа:
```json
{
  "code": 200,
  "data": {
    "items": [
      {
        "number": "1234",
        "amount": "15.000",
        "paymentType": "Payment by card",
        "paymentStatus": "New",
        "createdAt": "17.08.2023 17:00:33",
        "updatedAt": "18.08.2023 15:55:16"
      }
    ]
  }
}
```

---

### Получение информации о заказе
**URL**: `https://crypto-cash.world/merchant/api/v1/orders/retrieve/`

#### Параметры запроса:
- **`publicKey`**: Публичный ключ мерчанта
- **`number`**: Номер заказа

#### Пример ответа:
```json
{
  "code": 200,
  "data": {
    "item": {
      "number": "1234",
      "amount": "15.000",
      "paymentType": "Payment by card",
      "paymentStatus": "New",
      "createdAt": "17.08.2023 17:00:33",
      "updatedAt": "18.08.2023 15:55:16"
    }
  }
}
```

---

### Получение баланса
**URL**: `https://crypto-cash.world/merchant/api/v1/balance/retrieve/`

#### Параметры запроса:
- **`publicKey`**: Публичный ключ мерчанта

#### Пример ответа:
```json
{
  "code": 200,
  "data": {
    "item": {
      "accountAmount": "15.000",
      "companyAmount": "20.000"
    }
  }
}
```

---


### Список фиатных валют
**URL**: `https://crypto-cash.world/merchant/api/v1/fiat-currencies/list/`

#### Параметры (data):
- **`publicKey`**: Публичный ключ, связанный с аккаунтом мерчанта.

#### Пример ответа:
```json
{
  "code": 200,
  "data": {
    "items": [
      {"currency": "USD", "buy": false, "sale": true},
      {"currency": "EUR", "buy": false, "sale": true},
      {"currency": "UAH", "buy": true, "sale": false}
    ]
  }
}
```

Примечание:
buy : - признак использование валюты для продажи (false - нет, true - да)
sale: - признак использования валюты для покупки (false - нет, true - да)

---

### Запрос на покупку криптовалюты за фиатные валюты
**URL**: `https://crypto-cash.world/merchant/api/v1/mercuryo/form/retrieve/`

#### Параметры (data):
- **`publicKey`**: Публичный ключ, связанный с аккаунтом мерчанта.
- **`fiatAmount`**: Сумма покупки (используем валюту, которую покупаем).
- **`fiatCurrency`**: Фиатная валюта.
- **`currency`**: Криптовалюта.
- **`address`**: Номер криптокошелька.
- **`network`**: Подсеть криптовалюты. Не обязательный параметр (зависит от криптовалюты).
- **`redirectUrl`**: Ссылка для перенаправления после оплаты.
- **`externalId`**: Внешний ID.
- **`lang`**: Язык формы.

#### Пример ответа:
```json
{
  "code": 200,
  "data": {
    "item": {
      "externalId": "195",
      "url": "https://exchange.mercuryo.io/?widget_id=0b50150a-1ba0-445e-8637-d62f887afadc&address=...&hide_address=true&signature=...&fiat_currency=BGN&currency=USDT&networks=ERC20&fiat_amount=500.00&type=buy&fix_amount=true&fix_currency=true&fix_fiat_amount=true&fix_fiat_currency=true&merchant_transaction_id=...&lang=en&redirect_url=...",
    }
  }
}

```

---

### Поддерживаемые криптовалюты для Fiat On-Ramp iFrame API

| Currency | Network            |
|----------|--------------------|
| BTC      | BITCOIN            |
| ETH      | ETHEREUM           |
| ETH      | ZKSYNC             |
| ETH      | ARBITRUM           |
| ETH      | BASE               |
| ETH      | OPTIMISM           |
| ETH      | LINEA              |
| USDT     | ETHEREUM (ERC-20)  |
| USDT     | TRON (TRC-20)      |
| USDT     | SOLANA             |
| USDT     | POLYGON            |
| USDT     | NEWTON (TON)       |
| USDT     | BINANCESMARTCHAIN  |
| USDC     | ARBITRUM           |
| USDC     | STELLAR            |
| USDC     | SOLANA             |
| USDC     | BASE               |
| USDC     | POLYGON            |
| USDC     | NEAR_PROTOCOL      |
| USDC     | ETHEREUM           |
| SOL      | SOLANA             |
| XRP      | RIPPLE             |
| DOGE     | DOGECOIN           |
| TRX      | TRON               |
| LTC      | LITECOIN           |
| BCH      | BITCOINCASH        |
| AVAX     | AVALANCHE          |
| 1INCH    | BINANCESMARTCHAIN  |
| ADA      | CARDANO            |
| SHIB     | ETHEREUM           |
| POL      | POLYGON            |
| UNI      | ETHEREUM           |
| DOT      | POLKADOT           |
| ATOM     | COSMOS             |
| MANA     | ETHEREUM           |
| NEAR     | NEAR_PROTOCOL      |
| NOT      | NEWTON             |
| TON      | NEWTON             |

---

### Статусы заказов
- **Waiting**
- **Paid**
- **Not paid**

#### Дополнительно:
Для варианта оплаты криптовалютой используются все статусы. Также для оплаты криптовалютой есть минимальное время для обработки платежа. По умолчанию это время составляет 2 часа. Данное время может быть изменено по желанию сервиса.

---

### Ошибки
При возникновении ошибок в ответ будет возвращен список из кодов ошибок.

#### Пример ответа с ошибкой:
```json
{
  "errors": [1000]
}
```

#### Список ошибок:
- **1000** - Неверный запрос
- **2010** - Ошибка декодирования данных
- **2011** - Ошибка декодирования подписи
- **2020** - Неверная подпись
- **3000** - Компания не найдена
- **3001** - Заказ не найден

---

### Вебхуки
Сервис предоставляет возможность настроить вебхуки, чтобы получать уведомления о событиях, связанных с изменением статуса заказа. Для настройки вебхука необходимо в настройках аккаунта указать URL, на который будут отправляться данные.

#### Формат вебхука:
- **`data`**: Поле содержит объект JSON с информацией о заказе, закодированный в строку base64.
- **`signature`**: Подпись запроса, созданная на основе приватного ключа и данных (`SECRET_KEY` + `data`). Подпись кодируется в SHA256 и затем в base64.


#### Пример ответа на вебхук:

Запрос при изменении статуса заказа:

```json
{
     "data": "eyJpZCI6ICJlMGI5YTdiNS03ZDBkLTQ3YzUtODUzYS05ZTViNmJiNDBkMDUiLCAib3JkZXJJZCI6ICI3Nzc3IiwgInN0YXR1cyI6ICJQYWlkIiwgInVwZGF0ZWRBdCI6ICIyMDI0LTEwLTAxIDA5OjQxOjAwLjYwNjIzNyswMDowMCIsICJwdWJsaWNLZXkiOiAiYTJlMWI3YzgzODI2NGVmZTgyZmY1NzVlYzA3ZjhmYjQifQ==",
     "signature": ""
}
```

Запрос при изменении статуса заказа:

```json
{
     "id": "e0b9a7b5-7d0d-47c5-853a-9e5b6bb40d05",
     "orderId": "7777",
     "status": "Paid",
     "updatedAt": "2024-10-01 09:41:00.606237+00:00",
     "publicKey": "a2e1b7c838264efe82ff575ec07f8fb4",
}
```

